...
> 本文件记录 taskswarm 已知但尚未修复的问题。每条含现象、根因、相关代码位置与候选修复方案。
...
- **现象:** 批次 wave 1 全部 lane merged（含手动标记的失联 lane）后，wave 2 及后续 lane
  ...
  状态），但就是不调度；改 `.taskswarm/batches/*.json` 的 lane phase 不生效；`start` 因批次
...
- **修复说明（方案 B 看门狗超时 + 方案 A 磁盘恢复）:**
  ...
  - **方案 C（lane 级状态修正工具）未做**（可选）：`tswarm_supervisor_control` 的 `abort`
    工具已加**批次级防误伤提示**（scope 非空时明确"abort 是批次级、scope 不生效"并拒绝，
    防止误 abort 整批），单 lane 释放走 runbook §7.6 手动收尾（有 B 后无需重启）。
- **配置：** `index.ts` `Config` / `.taskswarm/config.json` 设 `laneTimeoutMinutes`（分钟，默认 90）。
...
- **差距（原）:** TaskPlane 有对话式 supervisor agent 与 operator 共享聊天会话，
  实时汇报 worker 进度并请求确认；TaskSwarm 移植时只移植了 mailbox 存储，
...
- **实现（v0.1.1 工作区，重启生效）:** `src/orchestrator/supervisor.ts`（TaskPlane
  ...
  - 会话 agent 即 supervisor：注入 supervisor 系统提示 + 两个工具
    （`tswarm_supervisor_status` 诊断、`tswarm_supervisor_control` 控制）
...
- **现象:** 旧 batch 的进程被 kill 后，`taskswarm/<taskId>` 分支与 lane worktree 目录残留；
  新 batch 的 `git worktree add -b taskswarm/<taskId>` 因分支已存在而失败，所有 lane 秒败。
- **修复说明:** `createLaneWorktree` 现在先移除残留 worktree 目录（失败则 `prune`），
  且当 `taskswarm/<taskId>` 分支已存在时改用 `git worktree add <branch> <dir>`（附着既有分支）
...
2. TaskSwarm worker 会话的 `cwd` 是 lane worktree（`.taskswarm/worktrees/<taskId>`），这是瞬态 git worktree，没有对应 workspace 记录 → 所有 worker 会话落入「未分组」兜底桶（`groupByWorkspace` 的 `stray`）。
3. DSH 内置 **subagent 会话自动隐藏**：侧边栏 `sessionVisible()` 过滤 `session.origin !== 'subagent'`。TaskSwarm 创建 agent 时未传 `meta.origin: 'subagent'`，导致这些内部会话暴露在列表里。
...
- **方案 A（推荐）:** `InProcessWorkerHost.spawn()` 与 `reviewer.ts` 的 `agents.create()` 增加 `meta: { origin: 'subagent', cwd: worktree, ... }`。效果：worker/reviewer 会话从侧边栏自动隐藏（符合"内部执行者"定位）；进度观察走 `/orch-status` 或未来 dashboard。
  - 代价：worker 会话在 GUI 不可见/不可点击（无法从侧边栏围观 worker），但进程内日志与 `.taskswarm` 状态仍在。
...
- 该问题不影响 batch 执行正确性（`b-msrszf33-d79cf6` 执行正常，lane 状态写入 `.taskswarm/batches/`），纯 UX 问题。
...
- 对比：README 记录的 `dsh --profile taskswarm-verify` 沙箱验证中，in-process worker 可正常建文件（该 profile 下 worker 有工具）。
...
- DSH agent 作用域的工具继承在 web profile 与 taskswarm-verify 沙箱 profile 间行为不同：
...
- **方案 B:** 默认 `host: headless`（`dsh --profile taskswarm-worker` 子进程，自带完整工具）。
- **方案 C:** 查证 web profile 下 agent 作用域工具继承机制，从 DSH 侧解决（若为框架行为差异）。

#### 复现步骤

1. web profile（GUI）会话里 `/tswarm-init` → `/orch all`
2. 观察 mailbox（`.taskswarm/mailbox/<batch>/supervisor/inbox/`）：worker escalate「Missing shell/file tooling」
...
- 2-lane 并发（wave-1）时，磁盘 `.taskswarm/batches/*.json` 里 lane 1（WEB-001）显示
...
2. `/orch all`
3. 读 `.taskswarm/batches/*.json`：先启动的 lane 可能显示 pending/无 worktree，尽管 worktree 已存在
