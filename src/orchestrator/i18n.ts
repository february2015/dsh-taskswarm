 ...
 *
 * Buju's supervisor notifications (event headlines, ETA, stall warnings,
 ...
 * switch the language at runtime by text (buju_supervisor_locale tool); the
 * choice is persisted to `<stateRoot>/config.json` so it survives restarts.
 * @module buju/orchestrator/i18n
 */
...
const zhCN: SupervisorMessages = {
  batchStarted: (id, total) => `[Buju supervisor] Batch ${id} 已启动（${total} 个任务）`,
  laneFailed: (lane, taskId, error) => `[Buju supervisor] ⚠️ lane ${lane} ${taskId} 失败${error ? `：${error}` : ''}`,
  laneRevise: (lane, taskId) => `[Buju supervisor] 🟡 lane ${lane} ${taskId} 待修订（reviewer REVISE）`,
  waveComplete: (wave, total, merged, failed) => `[Buju supervisor] 🌊 Wave ${wave}/${total} 完成：${merged} 成功 / ${failed} 失败`,
  batchComplete: (id, merged, total, failed) => `[Buju supervisor] ✅ Batch ${id} 完成：${merged}/${total} 成功，${failed} 失败`,
  batchAborted: (id) => `[Buju supervisor] ⛔ Batch ${id} 已中止`,
  etaLabel: '预计剩余：',
  noBatchState: '(no batch state)',
  stalled: (id, minutes) =>
    `[Buju supervisor] ⏱️ 疑似卡住：批次 ${id} 已约 ${minutes} 分钟无任何 lane 变化，且 worker 会话日志同样超时。` +
    '请用 buju_supervisor_status / 只读工具查证 lane 日志，判断是继续等待、pause 还是 abort。',
  periodicReport: (minutes) => `[Buju supervisor] ⏱️ 定时汇报（每 ${minutes} 分钟）：`,
...
const en: SupervisorMessages = {
  batchStarted: (id, total) => `[Buju supervisor] Batch ${id} started (${total} tasks)`,
  laneFailed: (lane, taskId, error) => `[Buju supervisor] ⚠️ lane ${lane} ${taskId} failed${error ? `: ${error}` : ''}`,
  laneRevise: (lane, taskId) => `[Buju supervisor] 🟡 lane ${lane} ${taskId} awaiting revision (reviewer REVISE)`,
  waveComplete: (wave, total, merged, failed) => `[Buju supervisor] 🌊 Wave ${wave}/${total} complete: ${merged} merged / ${failed} failed`,
  batchComplete: (id, merged, total, failed) => `[Buju supervisor] ✅ Batch ${id} complete: ${merged}/${total} merged, ${failed} failed`,
  batchAborted: (id) => `[Buju supervisor] ⛔ Batch ${id} aborted`,
  etaLabel: 'ETA: ',
  noBatchState: '(no batch state)',
  stalled: (id, minutes) =>
    `[Buju supervisor] ⏱️ Possibly stalled: batch ${id} has seen no lane changes for ~${minutes} minutes and worker session logs are stale too. ` +
    'Inspect lane logs with buju_supervisor_status / read-only tools, then decide: keep waiting, pause, or abort.',
  periodicReport: (minutes) => `[Buju supervisor] ⏱️ Periodic report (every ${minutes} min):`,
